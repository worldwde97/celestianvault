Нужно создать сайт Celestian Wallet с несколькими страницами. Дизайн на твое усмотрение, всё в одном стиле. Несколько страниц которые прилинкованы друг к другу. пример текста будет ниже, сделать рерайт, добавить \\ убрать секции на свое усмотрение, главное чтобы осталась суть. Картинки для дизайна пронумеруй от 1 до 10 и т.д. в порядке их использования сверху низ, затем я вставлю их вручную. кнопки все ведут на страницу регистрации или на логин. На странице регистрации всё должно работать, базу подкручивать не нужно. Страница регистрации работает условно, главное условие имя пользователя пароль почта. Если всё соответствует требованием, крутится загрузочный кружок 3 секунды и затем сообщение что Регистрация в "Celestian Wallet" запрещена в вашем регионе" всё на английском. Страница логина просто выдает, неправильный логин и пароль. Celestian сделать переменной чтобы в будущем можно было поменять быстро



**Home page:** 



Secure Multi Crypto Wallet



Safely store and swap your cryptocurrency

Create Wallet

apple

android

windows

hero image

Pay With Your Crypto Anywhere



Top up your balance with your cryptocurrency and use your wallet for storage and payments.

Get your Visa Card



&nbsp;   Store Crypto



&nbsp;   Store Bitcoin, Ethereum and over 30 other coins

&nbsp;   Get Started

&nbsp;   Exchange Crypto



&nbsp;   Instant crypto exchange for over 10 coins using your web wallet

&nbsp;   Exchange Now



All Assets In

One Multi Crypto Wallet



Use a convenient and secure crypto wallet to manage and swap your crypto

Smart Way To Manage Your Crypto



Celestian Wallet is an ultimate solution for modern crypto management. You can do it all from the comfort of one single web app. Replenish, Instantly Exchange at best rates, create few wallet addresses, add extra security layer with Multisignature, and more.









**webwallet page:**



**Web Crypto Wallet**



**Secure and convenient online crypto wallet for Bitcoin and other asset available anywhere at anytime. Store, and exchange cryptocurrencies, tokens, and stablecoins.**

**Create Web Wallet**

**hero image**

**What is**

**Online Crypto Wallet**



**Online Wallet is a wallet that you can access via the internet, typically through your browser. It is commonly known as a web wallet. Hot wallets are also available, but they are different types available. A web wallet is not always a hot wallet, but every hot wallet is a web wallet. Storing your Bitcoin in an online wallet is almost as secure as storing it in a cold wallet.**



**Web wallets take various forms, but the main component is a clean interface that has nothing to do with the interface. Aside from that, you can exchange your currencies safely and quickly.**

**Get** Celestian

**The Most Popular Cryptocurrencies.**

**One Web Wallet**



**Support for over 10 major cryptocurrencies of all leading protocols. Stay in complete control of your assets.**



    **10+ Assets ⚡️**

    **visa**

    **mc**

    **sepa**

    **Crypto Purchase**



    **Safe and Simple way to buy crypto with debit/credit card or a SEPA transfer directly in your wallet.**

    **Buy Crypto**

    **Limitless**



**Crypto Exchange**



**Instantly exchange Bitcoin and other cryptocurrencies and tokens without registrations, limits, or verification approvals.**

**Instant Exchange**

**Up To 25% APY**



    **Secure Wallet**



    **We use SHA 256 bit encryption for all transactions and actions in the web application and guarantee the complete security of your personal data and assets.**

    **Start Secure**



**Send**



**Easily manage you crypto online and send it to anyone and anywhere with their address.**

**Send Crypto**

**Spend**



**Using your** Celestian **Visa Card. Top up your card and spend your Bitcoins anywhere.**

**Prepaid Visa Card**

**Why Choose Us**



    **Available Everywhere**



    **Buy Bitcoin through web wallet all around the globe using the local currency of your bank account.**

    **Fast Payment**



    **Buy crypto with a credit / debit card, Apple Pay, Union Pay, via SEPA.**

    **Excellent Customer Service**



    **Online crypto wallet** Celestian **stands out for providing responsive and reliable customer service 24/7**

    **Desktop/Mobile Versions**



    **Use an online wallet, desktop wallet or download our mobile wallet app. Also, iOS and Android apps are available to manage your wallet on your phone.**



**Top Security Wallet For Your Crypto Assets**



Celestian Wallet **is a non-custodial wallet, so we don’t store private data, backup files, or keys. Only users have access to their personal data. Users are the only ones who have access to private information. No transactions can be tracked by third party. Fingerprint authentication and PIN protection are some of the available security features.**



**Multisigs feature is available (BTC, ETH)**

**2FA authentication \& Face ID**

**Back up files only you have access to**



    **Total privacy and security**



**Be In Charge of Your Crypto**



Celestian **is an ultimate solution for modern crypto management. You can do it all from the comfort of one single app. Earn with staking, Purchase, Instantly Exchange at best rates, get crypto loans, add extra security layer with Multisignature, and the list goes on.**

**Get Started**

**FAQ**



    **What is a web wallet for bitcoin and other crypto?**



    **Web crypto wallet is an online version of your wallet which you can use right in your browser.**

    **How do I create a crypto wallet online?**



    **OpenOnline Crypto Wallet page on Celestian; Click on “Get started”; Create a new wallet or unlock it with your password**

    **What are the advantages of using the online bitcoin wallet for bitcoin and other crypto?**



    **You can easily access all assets right in your browser. It is convenient to use the online wallet at any time.**

    **Are crypto web wallets safe?**



    **Web wallets are all typically hot wallets .Web wallets are the least safe, though all crypto hot wallets are quite vulnerable to online attacks. A benefit to hot wallets is ease-of-use.**





**Exchange:** 





Fast Crypto Exchange



Celestian Wallet platform is the fastest way to exchange any crypto-assets without registrations, KYC and long waits. It is one of the best crypto to crypto exchanges that established trust for a long time in the market.

hero image

Advantages of Using Crypto Coins Exchange



&nbsp;   No Limits

&nbsp;   No Limits

&nbsp;   We do not set any limits, you can make as many exchanges as you need.

&nbsp;   Private

&nbsp;   Private

&nbsp;   You do not have to go through an authorization procedure, so all your exchanges will be completely private.

&nbsp;   Fast

&nbsp;   Fast

&nbsp;   The exchanges that take place at Celestian Wallet are made in a matter of minutes, allowing you to save time.

&nbsp;   Fair

&nbsp;   Fair

&nbsp;   Our exchange rates are carefully selected so that our customers are satisfied and know that our offers are among the best on the market.



Trusted and secure  Bitcoin \& crypto exchange

Celestian Wallet platform is the fastest way to exchange any crypto-assets without registrations, KYC and long waits. It is one of the best crypto to crypto exchanges that established trust for a long time in the market.

If You’re New to Celestian Wallet Crypto Exchange Wallet

Our task is to conduct safe and quality exchanges because we focus on our customers and strictly control the processes of our clients' time. Celestian Wallet cryptocurrency exchange online doesn't just work to provide services, but our team is constantly working to simplify and speed up the adoption of crypto assets in everyday life. Therefore, we understand to what extent it is important to create an ecosystem in which it will be convenient to conduct transactions and give clients the best exchange rates.

Sign Up





About:







About Us



Celestian Wallet is web Wallet, that supports the most popular coins and the others.

10+

Assets Supported

4+

Products \& Services

24/7

Live Support

Fixie Wallet Mission



Celestian Wallet developers created digital wallets that provide complete security. Everyone can use our software for easy storing and managing cryptocurrencies. We respect, appreciate, and sincerely love the blockchain technology. Our priority is the decentralization of the Fixie Wallet software in accordance with the basic principles of blockchain.



User privacy and safety are the main postulates for us, that is why we do not store your private keys, backup files, or any other data on Fixie Wallet servers. All your private data is stored only by you. Our wallet is lightweight, has a convenient and intuitive interface.



&nbsp;   Our core values are freedom of privacy and top level security.



Milestones



Celestian Wallet has started with single currency mono wallets, and now our products available from any browser, any operating system of your PC (Windows, macOS, Linux/Ubuntu), and smartphone (iOS and Android web).



&nbsp;   01

&nbsp;   01-icon



&nbsp;   Started with single currency wallets

&nbsp;   02

&nbsp;   02-icon



&nbsp;   Added the aditional secure layers

&nbsp;   03

&nbsp;   03-icon



&nbsp;   Web super secure version

&nbsp;   04

&nbsp;   04-icon



&nbsp;   Token Generator, Bitcoin Multisigs support

&nbsp;   05

&nbsp;   05-icon



&nbsp;   Multi currency, crossplatformt



Stay in Touch

Support 24/7

Learn about blockchain, cryptocurrencies, DeFi, wallets and get useful guides.

Start Learning

Newsletter

We send a brief email usually once every two weeks with news, giveaways, and updates. We'll never share your address with any third party.

Our Blog

Read our blog to stay up to date on Fixie news, new features, and press releases.

Read Blog





